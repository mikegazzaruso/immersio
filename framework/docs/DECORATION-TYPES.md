...
All procedural decoration types available in level configs. Decorations are placed in the `decorations` array and rendered by `LevelLoader` via the `DecorationRegistry`.
...
Games can register custom decoration types via the `DecorationRegistry`:

```js
// In Engine.js or a setup file, after engine construction:
engine.decorationRegistry.register('torch', (scene, config, env, ctx) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    scene.add(torch);
    ctx.objects.push(torch); // required for cleanup on level change
  }
});
```
...
```js
(scene, config, env, ctx) => void
```

| Param | Description |
|---|---|
| `scene` | THREE.Scene — add objects here |
| `config` | The decoration entry from the level config (`{ type, count, ... }`) |
| `env` | The full `environment` object from the level config |
| `ctx` | Context: `{ objects, birds, waterPlanes, lanterns }` — push created objects to `ctx.objects` for cleanup. Use `ctx.birds/waterPlanes/lanterns` for animated types that LevelLoader animates in `update()` |
