...
### Linear Chain (default — backward compatible)

If no puzzle has `dependencies` set, PuzzleManager chains them linearly in registration order:
...
### Dependency Graph (non-linear puzzles)

Set `puzzle.dependencies` to an array of puzzle IDs that must be solved before this puzzle activates. PuzzleManager detects graph mode automatically when any puzzle has dependencies.

```js
// Example: puzzleA and puzzleB are independent (start active),
// puzzleC requires both A and B solved first

const puzzleA = new CollectPlacePuzzle(this.eventBus, this.scene, this.interactionSystem);
// puzzleA.dependencies = []; // no deps → root node (starts active)

const puzzleB = new SequencePuzzle(this.eventBus, this.scene, this.interactionSystem);
// puzzleB.dependencies = []; // no deps → root node (starts active)

const puzzleC = new TriggerAnimPuzzle(this.eventBus, this.scene, this.collisionSystem);
puzzleC.dependencies = ['collect_place', 'sequence']; // requires both A and B

this.puzzleManager.register(puzzleA);
this.puzzleManager.register(puzzleB);
this.puzzleManager.register(puzzleC);
this.puzzleManager.init();
// → puzzleA and puzzleB activate immediately
// → when BOTH are solved, puzzleC activates
// → when puzzleC is solved, game:complete fires
```

### Dependency Patterns

| Pattern | Dependencies | Description |
|---|---|---|
| Linear | `[]`, `[A]`, `[B]` | Classic chain: A → B → C |
