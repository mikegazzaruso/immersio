...
export default {
  ...
  exit: { ... },            // Optional: portal(s) to other levels
};
```
...
To connect levels, add an `exit` field. The engine renders a glowing portal and triggers a fade transition when the player walks close.

```js
exit: {
  position: [0, 0, -15],        // Portal position [x, y, z]
  targetLevel: 2,                // Level to load
  label: 'Enter the Cave',      // Optional: label (for HUD)
  rotationY: 0,                  // Optional: portal facing direction
  color: '#44aaff',              // Optional: portal glow color (default blue)
}
```

Multiple exits (branching paths):

```js
exit: [
  { position: [5, 0, -10], targetLevel: 2, label: 'Path A' },
  { position: [-5, 0, -10], targetLevel: 3, label: 'Path B' },
]
```

**Transition flow:** Player enters proximity (1.8m) → fade to black (0.6s) → load target level → fade in (0.6s).
...
export default {
  ...
  exit: { position: [0, 0, -20], targetLevel: 2, label: 'To the Cave' },
};
```
