...
export class VRSetup {
  ...
  init() {
    ...
    // Add visible ray lines to controllers
    this._ray0 = this._createRayLine();
    this.controller0.add(this._ray0);
    this._ray1 = this._createRayLine();
  ...
  setRayHover(hand, hovering) {
    const ray = hand === 'left' ? this._ray0 : this._ray1;
    if (!ray) return;
    const color = hovering ? 0x00ffcc : 0x44aaff;
    const opacity = hovering ? 0.8 : 0.5;
    ray.material.color.setHex(color);
    ray.material.opacity = opacity;
    const dot = ray.children[0];
    if (dot) dot.material.color.setHex(color);
  }
